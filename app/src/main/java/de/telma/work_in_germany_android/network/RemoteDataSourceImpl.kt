...
class RemoteDataSourceImpl(
    ...
    override suspend fun fetchJobs(): RemoteJobsResponse = withContext(Dispatchers.IO) {
        val response = try {
        ...
        when (response.status.value) {
            in 200..299 -> return@withContext response.body() as RemoteJobsResponse
            else -> throw JobFetchingException("Failed to fetch jobs: ${response.status}")
        }
    }

    @Throws(StatsFetchingException::class)
    override suspend fun fetchStats(): RemoteStats = withContext(Dispatchers.IO) {
        val response = try {
        ...
        when (response.status.value) {
            in 200..299 -> return@withContext response.body() as RemoteStats
